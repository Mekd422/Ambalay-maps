...
export const verifySignup = async (data: {
  ...
  const res = await API.post<ApiResponse<AuthVerificationResult>>(
    '/auth/signup/verify',
    data,
  )
  return res.data.data
...
export const resendOtp = async (data: { challengeId: string }) => {
  const res = await API.post<ApiResponse<OtpChallenge>>(
    '/auth/otp/resend',
    data,
  )
  return res.data.data
...
export const resetPassword = async (data: {
  ...
  const res = await API.post<ApiResponse<Record<string, never>>>(
    '/auth/reset_password',
    data,
  )
  return res.data
